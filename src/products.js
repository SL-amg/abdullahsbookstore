

const products = [

{
    name:"Twilight Breaking Dawn",
    price:"10 KD",
    image:"https://upload.wikimedia.org/wikipedia/en/3/31/Breaking_Dawn_cover.jpg"
},
{
    name:"Charlie and the Chocolate",
    price:"12 KD",
    image:"https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1309211401i/6310.jpg"
},
{
    name:"JavaScript for Dummies",
    price:"15 KD",
    image:"https://m.media-amazon.com/images/I/61Ll+ZKJc5L._AC_UF1000,1000_QL80_.jpg"
},
{
    name:"Electrical Engineering",
    price:"100 KD",
    image:"https://m.media-amazon.com/images/I/61VkoauwY1L._AC_UF1000,1000_QL80_.jpg"
},
{
    name:"Harry Potter",
    price:"0.500 KD",
    image:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSXmow8QGoUxQCbdE70iUNub8ObBYINje0kpg&s"
},
{
    name:"Batman",
    price:"1000 KD",
    image:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS3aducgTJ2RORwcAj7Ij6kS8-ppLuY9LDQ7Q&s"
},
]

export default products; // to export the array

// to export multiple, we dont use default and use { xx , yyy} and in main App.js file we call it Import {xxx,yyy} then we can shift everything to this file (i mean map file) //